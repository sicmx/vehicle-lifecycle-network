...
describe('Starting tests for vehicle-lifecycle-network-app', function() {
  ...
  it('website title should be vehicle-lifecycle-network-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('vehicle-lifecycle-network-app');
    })
  });

  it('network-name should be vehicle-lifecycle-network@0.1.11-20171020160718',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('vehicle-lifecycle-network@0.1.11-20171020160718.bna');
    });
  });

  it('navbar-brand should be vehicle-lifecycle-network-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('vehicle-lifecycle-network-app');
    });
  });
